...
- у кнопок появились состояния нажатия и фокуса с клавиатуры, а панель открывается коротким появлением на 170 мс, которое отключается при `prefers-reduced-motion`;
- прозрачность панели настраивается от 50 до 100 процентов. Просвечивает только фон: текст, рамки и тени остаются плотными, а страница под панелью размывается, поэтому перевод читается и на пестром сайте.
